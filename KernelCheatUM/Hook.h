...
#include <Uxtheme.h>
HANDLE Mem;
LPCSTR TargetProcess = "crossfire.exe";
bool ShowMenu = true;
...
void Draw() {
	...
	DrawStrokeText(30, 44, &Yellow, FpsInfo);
}

void Render() {

	if (!EngineInitAddress)
		EngineInitialize();
...
bool InitializeMenu() {
	...
	OverlayWindow::Name = random_string(10).c_str();
	SetupWindow();
	DirectXInit();
	Mem = CreateThread(0, 0, ProcessCheck, 0, 0, 0);
	while (TRUE) {
		MainLoop();
